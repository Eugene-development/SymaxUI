<script>
    import { form_03_sent } from '../../../store/stores.js';
    ...
    async function sendConsultation() {
        try {
            const payload = { name, phone, variant };
    ...
    const sendForm_03 = () => {
        try {
            form_03_sent.update(invertToTrue);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    <div class="bg-white sm:max-w-md sm:w-full sm:mx-auto sm:rounded-lg sm:overflow-hidden">
        <div class="px-4 py-8 sm:px-10">
            ...
            <div class="mt-6 relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="px-2 bg-white text-gray-500 "> ответим в течении рабочего дня </span>
                </div>
            </div>
            ...
            <div class="mt-6">
                <form on:submit|preventDefault|once={sendConsultation} class="space-y-6">
                    ...
                    <div>
                        <label for="variant" class="sr-only">Интересуемый вид</label>
                        <input
                                bind:value={variant}
                                type="text"
                                name="variant"
                                ...
                                placeholder="Нужный вид перекиси"
                                required
                                class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md"
                        />
                    </div>
                    ...
                    <div>
                        <label for="volume" class="sr-only">Нужный объём</label>
                        <input
                                bind:value={volume}
                                type="text"
                                name="volume"
        ...
        <div class="px-4 py-6 bg-gray-50 border-t-2 border-gray-200 sm:px-10">
            <p class="text-xs leading-5 text-gray-500 text-center">
                После отправки заявки с вами свяжется менеджер для уточнения нюансов
            </p>
        </div>
    </div>
</div>
